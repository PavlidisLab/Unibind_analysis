...
meta_rob_mm <- read.delim("~/Data/Metadata/Chipseq/unibind_robust_mouse.tsv", stringsAsFactors = FALSE)
...
meta_perm_mm <- filter(meta_perm_mm, N >= 100)
meta_rob_mm <- filter(meta_rob_mm, N >= 100)


# Scoring all is slow - took ~ 30 hours on frink with 8 cores

if(!file.exists("~/scratch/R_objects/unibind_all_scores_mm.RDS")) {
  bscore_mm <- load_and_score(path_mm, meta_perm_mm, bl_gr_mm, pc_gr_mm)
  names(bscore_mm) <- meta_perm_mm$File
...
bmat_mm <- bmat_mm[, meta_rob_mm$File]


# Looking for duplicate samples with multiple motifs


meta_dedup_mm <- meta_rob_mm[meta_rob_mm$ID %in% meta_rob_mm$ID[duplicated(meta_rob_mm$ID)], ]
...
meta_final_mm <- filter(meta_rob_mm, !(ID %in% meta_dedup_mm$ID)) %>% 
  rbind(meta_dedup_mm) %>% 
  arrange(Symbol)
...
saveRDS(list(
  ...
  Fit = top_mm),
  file = "~/scratch/R_objects/unibind_bindscore_robust_mouse.RDS"
)
