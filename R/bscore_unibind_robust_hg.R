...
meta_rob_hg <- read.delim("~/Data/Metadata/Chipseq/unibind_robust_human.tsv", stringsAsFactors = FALSE)
...
meta_perm_hg <- filter(meta_perm_hg, N >= 100)
meta_rob_hg <- filter(meta_rob_hg, N >= 100)


# Scoring all is slow - took ~ 30 hours on frink with 8 cores

if(!file.exists("~/scratch/R_objects/unibind_all_scores_hg.RDS")) {
  bscore_hg <- load_and_score(path_hg, meta_perm_hg, bl_gr_hg, pc_gr_hg)
  names(bscore_hg) <- meta_perm_hg$File
...
bmat_hg <- bmat_hg[, meta_rob_hg$File]


# Looking for duplicate samples with multiple motifs


meta_dedup_hg <- meta_rob_hg[meta_rob_hg$ID %in% meta_rob_hg$ID[duplicated(meta_rob_hg$ID)], ]
...
meta_final_hg <- filter(meta_rob_hg, !(ID %in% meta_dedup_hg$ID)) %>% 
  rbind(meta_dedup_hg) %>% 
  arrange(Symbol)
...
saveRDS(list(
  ...
  Fit = top_hg),
  file = "~/scratch/R_objects/unibind_bindscore_robust_human.RDS"
)
